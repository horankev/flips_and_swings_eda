...
Throughout the study, `con_19` will refer to the percentage of legitimate votes which were cast for the Conservatives (along with equivalent variables such as `con_17`, `lab_19` and `lab_17`). The measure of swing which will be used is called the "*Butler swing*", hereafter abbreviated to `con_swing`, and is calculated as the mean of the difference between the change in percentage points for the Conservatives from 2017-2019 and the change in percentage points for Labour.
...
plot(swing.nb, df_nb, add = TRUE, pch = ".", col = "darkred", lwd=0.5)
...
plot(lon, border="black")
plot(swing.nb2, lon, add = TRUE, pch = ".", col = "darkred", lwd=1)
...
lisa1 <- local_moran(queen_w, df["PC1"]) # or any other variable :)

# process results
df$clusterl1 <- as.factor(lisa1$GetClusterIndicators())
levels(df$clusterl1) <- lisa1$GetLabels()

# A visual overview
lisaPC1 <- ggplot() + 
  geom_sf(data=gb, lwd=0.8) +
  geom_sf(data = df, aes(fill = clusterl1), lwd=0.05, colour="black") + 
  geom_sf(data = regions, lwd = 0.5, fill=NA) + 
  #geom_sf_label_repel(data = regions, aes(label=region),size=2, fontface="bold") + 
  scale_fill_manual(values = c("white","darkred","darkblue","lightblue","pink","grey")) + 
  labs(title = "LISA plot: PC1",
       subtitle = "HIGH => younger, with lower density, fewer students & born_elsewhere") + 
  theme_bw() + 
  theme(plot.subtitle = element_text(size=8))

lisa2 <- local_moran(queen_w, df["PC2"]) # or any other variable :)

# process results
df$clusterl2 <- as.factor(lisa2$GetClusterIndicators())
levels(df$clusterl2) <- lisa2$GetLabels()

# A visual overview
lisaPC2 <- ggplot() + 
  geom_sf(data=gb, lwd=0.8) +
  geom_sf(data = df, aes(fill = clusterl2), lwd=0.05, colour="black") + 
  geom_sf(data = regions, lwd = 0.5, fill=NA) + 
  #geom_sf_label_repel(data = regions, aes(label=region),size=2, fontface="bold") + 
  scale_fill_manual(values = c("white","darkred","darkblue","lightblue","pink","grey")) + 
  labs(title = "LISA plot: PC2",
       subtitle = "HIGH => more qualifications, less deprived") + 
  theme_bw() + 
  theme(plot.subtitle = element_text(size=8))
...
```{r, fig.width=10, fig.height=5}
lisaPClond1 <- ggplot() + 
  geom_sf(data = df |> filter(region=="London"), aes(fill = clusterl1), lwd=0.05, colour="black") + 
  geom_sf(data = regions |> filter(region=="London"), lwd = 0.5, fill=NA) + 
  #geom_sf_label_repel(data = regions, aes(label=region),size=2, fontface="bold") + 
  scale_fill_manual(values = c("white","darkred","darkblue","lightblue","pink","grey")) + 
  labs(title = "LISA London plot: PC1",
       subtitle = "HIGH => younger, with lower density, fewer students & born_elsewhere") + 
  theme_bw() + 
  theme(plot.subtitle = element_text(size=8))

lisaPClond2 <- ggplot() + 
  geom_sf(data = df |> filter(region=="London"), aes(fill = clusterl2), lwd=0.05, colour="black") + 
  geom_sf(data = regions |> filter(region=="London"), lwd = 0.5, fill=NA) + 
  #geom_sf_label_repel(data = regions, aes(label=region),size=2, fontface="bold") + 
  scale_fill_manual(values = c("white","darkred","darkblue","lightblue","pink","grey")) + 
  labs(title = "LISA London plot: PC2",
       subtitle = "HIGH => more qualifications, less deprived") + 
  theme_bw() + 
  theme(plot.subtitle = element_text(size=8))


plot_grid(lisaPClond1, lisaPClond2)
```
...
ggplot(flipdf,aes(party,`% votes`,fill=party)) + geom_col(aes(colour=winner,size=winner), show.legend = FALSE) + 
  scale_size_manual(values = c(0.1,3,0.1,3,0.1,0.1)) + 
  scale_fill_manual(values = rep(c("darkblue","darkred","gray25"),2)) + 
   scale_colour_manual(values = c("black","darkorange1","black","black","black","darkorange1")) + 
 facet_wrap(~year) + 
  theme_bw() +
